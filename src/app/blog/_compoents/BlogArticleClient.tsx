...
import { ArrowLeft, CalendarDays, Clock3, Eye, ExternalLink, Share2, UserRound } from "lucide-react";
import type { BlogPost } from "../_data/data";
    ...
    <main className="relative min-h-screen overflow-hidden bg-slate-950 pt-[130px] text-white">
      ...
      <article className="relative z-10 pb-24">
        ...
        >
          ...
          <a href={post.postUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-400 transition hover:text-cyan-200">
            View original archive <ExternalLink size={14} />
          </a>
        </motion.header>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, delay: 0.1 }} className="mx-auto max-w-6xl px-6 lg:px-10">
          <div className="relative aspect-[16/8] overflow-hidden rounded-3xl border border-white/15 shadow-[0_26px_70px_rgba(2,6,23,0.6)]">
            <Image src={post.images.main} alt={post.images.alt || post.title} fill priority sizes="(max-width: 1200px) 100vw, 1152px" className="object-cover" />
        ...
        <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.18 }} className="mx-auto mt-12 grid max-w-5xl gap-10 px-6 lg:grid-cols-[120px_minmax(0,1fr)] lg:px-10">
          <aside className="hidden lg:block">
            <div className="sticky top-36 flex flex-col items-center gap-4">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Share</span>
              <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://www.visualytes.com/blog/${post.blogKey}`)}`} target="_blank" rel="noreferrer" aria-label="Share on LinkedIn" className="rounded-full border border-white/15 bg-white/5 p-3 text-cyan-200 transition hover:border-cyan-300/50 hover:bg-cyan-300/10"><Share2 size={17} /></a>
            </div>
          </aside>
