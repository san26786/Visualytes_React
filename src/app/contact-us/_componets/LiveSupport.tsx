...
const FALLBACK_IMAGE = "/assets/jpg/bg_3.jpg";

export default function LiveSupport({ content }: { content: ContactContent["liveSupport"] }) {
  const image = content.image || FALLBACK_IMAGE;
  const dial = content.callNumber.replace(/[^\d+]/g, "");
  const whatsapp = content.whatsappNumber.replace(/\D/g, "");
        ...
        <div className="relative overflow-hidden rounded-[2.25rem] border border-white/15 shadow-[0_30px_80px_rgba(0,0,0,0.55)]">
          ...
          <div className="grid min-h-[560px] grid-cols-1 lg:grid-cols-2">
            ...
            >
              ...
              >
                <span className="text-[18px] font-semibold text-white">{content.callLabel}</span>
                <a
                  href={`tel:${dial}`}
                ...
                {whatsapp && <span className="text-slate-400">or…</span>}
              </motion.div>
            ...
            >
              ...
              >
                <p className="text-[11px] uppercase tracking-widest text-slate-400">{content.responseTimeLabel}</p>
                <p className="mt-0.5 text-[22px] font-bold text-white">{content.responseTimeValue}</p>
              </motion.div>
              ...
              >
                <p className="text-[11px] uppercase tracking-widest text-slate-400">{content.availabilityLabel}</p>
                <p className="mt-0.5 text-[18px] font-bold text-emerald-300">{content.availabilityValue}</p>
              </motion.div>
            </motion.div>
