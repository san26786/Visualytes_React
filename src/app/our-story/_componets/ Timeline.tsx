...
export default function Timeline() {
  ...
  useEffect(() => {
    ...
    const start =
      firstDot.left - container.left + firstDot.width / 2;
  
    // Line starts from the center of 2009
    ...
    const end =
      currentDot.left - container.left + currentDot.width / 2;
  
    // Only draw from 2009 to current year
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="pointer-events-none absolute bottom-[-60] left-1/2 -translate-x-1/2 whitespace-nowrap select-none text-[180px] md:text-[260px] font-black uppercase text-pink-400/5 "
      >
        OUR STORY
      </motion.div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 ">
        ...
        <div className="relative mt-24 overflow-hidden ">
          <div className="relative" ref={containerRef}>
            {/* Base Line */}
            <div
  className="absolute top-[52px] h-[2px] bg-white/20"
  style={{
    left: progressLeft,
...
/>
            {/* Progress Line */}
            <motion.div
  className="absolute top-[52px] h-[2px] bg-white"
  animate={{
    left: progressLeft,
    width: progressWidth,
  }}
              ...
              {timeline.map((item, index) => {
                ...
                return (
                  <div
                    key={item.year}
                    onClick={() => setActive(index)}
                    className="flex w-[220px] flex-shrink-0 cursor-pointer flex-col items-center"
                  >
                    <span
                      className={`mb-6 text-sm font-semibold tracking-[3px] transition-all duration-300 ${
                        activeItem ? "text-white" : "text-gray-500"
                      }`}
                    >
                      {item.year}
                    </span>
                    ...
                    <motion.div
                      ...
                      className={`relative z-10 h-4 w-4 rounded-full border-4 transition-all duration-500 ${
                        activeItem
                          ? "border-[#ff9cb7] bg-[#ff4d73] shadow-[0_0_10px_#ff4d73]"
                          : "border-[#374151] bg-[#ff4d73]"
                      }`}
                    />
                  </div>
                );
